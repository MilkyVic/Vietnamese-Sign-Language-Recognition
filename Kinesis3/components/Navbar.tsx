...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Initialize on mount so the effect matches the current scroll position (old behavior).
    handleScroll();
    ...
    >
      <div className="container mx-auto px-6 lg:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 md:gap-4">
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden shadow-lg shadow-primary-500/20 bg-white border border-slate-100 flex items-center justify-center">
            <img src="/logo.png" alt="Kinesis Logo" className="w-full h-full object-cover scale-110" />
          </div>
          <div className="flex flex-col">
            <span className="font-bold text-lg md:text-xl uppercase tracking-wide text-slate-900 leading-none">Kinesis</span>
            <span className="text-[10px] md:text-xs font-medium text-slate-500 uppercase tracking-wider">For Deaf Community in Danang</span>
          </div>
        </div>
