...
def create_action_folder(data_path, action, action_dirs):
    if action not in action_dirs:
        base_name = sanitize_action_name(action)
        candidate = base_name
        suffix = 1
    ...
    action_path = os.path.join(data_path, folder_name)
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc='Process actions'):
        ...
        action_path = create_action_folder(DATA_PATH, action, action_directories)
